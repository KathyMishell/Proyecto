...
#define tam_sig 90 //tamaño de la señal orginal
#define tam_imp 21 //tamaño del impulso

//Importar la señal 
extern double short_InputSignal_1kHz_15kHz[tam_sig];                  //Importamos o llamamos la señal original
extern double Impulse_response[tam_imp];                              //Importamos o llamamos la señal impulso

double output[tam_sig+tam_imp];                                       //declaramos vector de salida
double mediana_signal[tam_sig];                                       //declaramos vector de salida del suavizado 
...
void loop() {
  convolucion((double *)&short_InputSignal_1kHz_15kHz[0], (double *)&output[0], (double *)&Impulse_response[0],   //Llamamos el método de convolución
                   (int) tam_sig, (int) tam_imp);
  mediana ((double *)&short_InputSignal_1kHz_15kHz[0], (double *)&mediana_signal[0], (int) tam_sig);              //Llamamos el método de mediana
...
void SNR(){                                   //Método de SNR
  ...
  for(int i=0;i<90;i++){                             //ciclos recorre el numero de datos d ela señal
      sum = sum + short_InputSignal_1kHz_15kHz[i];   //guarda los valores obtenidos
    }
...
void plot_signal(void){ 
  int i;
  for(i=0;i<tam_sig;i++){
    Serial.print(String("Señal Original: ")+String(short_InputSignal_1kHz_15kHz[i]+600));
    Serial.print(",");
    Serial.print(String("Filtrado: ")+String(mediana_signal[i]+300));
    Serial.print(",");
    Serial.println(String("Suavizado: ")+String(output[i]));
    delay(5);
  }
}
